...
- **Obsidian flavour** — `[[wikilinks]]` with Obsidian's own resolution rules, callouts, `==highlight==`, wiki-embeds, tables, task lists, footnotes (`[^1]` and inline `^[…]`) that open where they are cited, block references — `[[Note#^id]]` scrolls to the block and `![[Note#^id]]` embeds just it — and `[[Note#Chapter#Section]]` heading paths
...
**A task can be pushed back.** Holding a task — in the task list, or its box in a note — offers tomorrow, the weekend, next Monday or a date. The date the task is filed under — due, else scheduled — goes to the chosen day, so the task always leaves the group it was in; its other dates (due, scheduled, start) move by the same number of days, as a repeat's do, so a task scheduled three days before it is due stays that way. With neither date on the line, a scheduled date is added. Created and done dates, a repeat rule and a block id stay where they are. The move can be undone from the snackbar, because it can be undone exactly: the dates go back by the same distance, or the added one comes off.
